...
while(True):
    ...
    try:
        ...
        if g==0:
            ...
            dst1 = cv2.undistort(gray1, cm1, dc1)                        
            dst2 = cv2.undistort(gray2, cm2, dc2)                        


            corners1, ids1, rejectedImgPoints1 = aruco.detectMarkers(dst1, aruco_dict, parameters=parameters)
            corners2, ids2, rejectedImgPoints2 = aruco.detectMarkers(dst2, aruco_dict, parameters=parameters)    
            ...
            P11 = np.zeros(shape=(3, 4))
            rot_trans_mat = np.zeros(shape=(3, 4))
            rot_trans_mat = np.concatenate((R1, T1), axis=1)
            # print(rot_trans_mat)

            P11 = np.matmul(cm1,rot_trans_mat)
            # print(P11)
            # print(P1)
            # print()
            exit()
